...
angular.module('lotes', ['ngRoute'])
    ...
    .controller('lotesCtrl', [
        ...
        function ($scope, store, $location, growl, $http, event, $route) {
            ...
            $scope.editLote = function (lote) {
                $scope.formData = lote;
                $scope.formLote = true;
            };

            $scope.editValor = function (lote, valor) {
                $scope.formValor = valor;
            ...
            $scope.inativaValor = function (lote, valor) {

                $http.post(window.global.url + 'api/financeiro/valor/inativar',
                    {
                        idValor: valor.id
                    }
                ).success(function (data) {
            ...
            $scope.inativarLote = function (lote) {
                $http.post(window.global.url + 'api/evento/lote/inativar',
                    {
                        idLote: lote.id
                    }
                ).success(function (data) {
            ...
            $scope.saveLote = function() {
                $scope.formData.dataInicio = $('#dtInicio').val().replace(/\//g, '-');
                $scope.formData.dataFim = $('#dtFim').val().replace(/\//g, '-');
